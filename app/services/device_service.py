...
class SimpleDeviceService:
    ...
    def get_device_time(self, auto_sync: bool = False) -> Dict[str, Any]:
        """Read device clock. `auto_sync` is deprecated and ignored — clock
        adjustment now happens exclusively in the background scheduler via
        ZkClient, eliminating the previous duplicate time-sync path."""
        device = self.get_default_device()
        if not device:
