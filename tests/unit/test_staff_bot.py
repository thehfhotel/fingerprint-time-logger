...
class TestSlotWebhookIntegration:
    ...
    def test_the_relay_is_not_handed_a_token_the_slot_digest_claimed(
        self, test_client, test_db, staff_oa_enabled, slot_bot, staff, monkeypatch
    ):
        forwarded = []
        monkeypatch.setenv("GUEST_FEEDBACK_LINE_URL", "http://guest-feedback:4090/line")
        monkeypatch.setattr(
            service, "forward_group_events_in_background", forwarded.extend
        )
        ...
        assert forwarded and forwarded[0].get("replyToken") in (None, "")
