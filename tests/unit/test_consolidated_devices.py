...
class TestConsolidatedDevicesAPI:
    ...
    def test_get_devices_list(self, test_client, test_company_setup):
        """Test GET /api/private/devices/ - List all devices"""
    ...
    def test_get_device_not_found(self, test_client):
        """Test GET /api/private/devices/{device_id} - Device not found"""
        response = test_client.get("/api/private/devices/999")
        assert response.status_code == 404



    def test_update_device(self, test_client, test_company_setup):
        """Test PUT /api/private/devices/{device_id} - Update device"""
        ...
        if devices:
            ...
            response = test_client.put(f"/api/private/devices/{device_id}", json=update_data)
            assert response.status_code in [200, 404]

    def test_delete_device(self, test_client, test_company_setup):
        """Test DELETE /api/private/devices/{device_id} - Delete device (expects 400 if has records)"""
        devices = test_company_setup["devices"]
        if devices:
            device_id = devices[0].id
            response = test_client.delete(f"/api/private/devices/{device_id}")
    ...
    def test_device_health_check(self, test_client):
        """Test GET /api/private/devices/health - Health check"""
    ...
    def test_sync_device_time(self, test_client, test_company_setup):
        ...
        if devices:
            device_id = devices[0].id
            response = test_client.post(f"/api/private/devices/{device_id}/sync-time")
            assert response.status_code in [200, 404, 500]

    def test_get_device_status(self, test_client, test_company_setup):
        """Test GET /api/private/devices/{device_id}/status - Get device status"""
        devices = test_company_setup["devices"]
        if devices:
            device_id = devices[0].id
            response = test_client.get(f"/api/private/devices/{device_id}/status")
            assert response.status_code in [200, 404]
    ...
    def test_get_device_diagnostics(self, test_client, test_company_setup):
        ...
        if devices:
            device_id = devices[0].id
            response = test_client.get(f"/api/private/devices/{device_id}/diagnostics")
            assert response.status_code in [200, 404, 500]
...
class TestConsolidatedDevicesAPIEdgeCases:
    ...
    def test_update_nonexistent_device(self, test_client):
        ...
        response = test_client.put("/api/private/devices/999", json=update_data)
        assert response.status_code == 404

    def test_delete_nonexistent_device(self, test_client):
        """Test deleting non-existent device"""
        response = test_client.delete("/api/private/devices/999")
        assert response.status_code == 404
